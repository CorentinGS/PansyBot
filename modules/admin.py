...
class Admin(commands.Cog):
    ...
    async def cog(self, ctx):
        await ctx.send(embed=func.eErr("Choose one",
                                       "**cog load** - Load a cog.\n**cog unload** - Unload a cog.\n**cog reload** - Reload a cog\n**cog list** - Lists all cogs.",
                                       "Cogs"), delete_after=config.deltimer)
        ...
        if cog:
            if cog != "admin":
                try:
                    ...
                    await ctx.send(embed=func.Editable("Done", f"{cog} was successfully unloaded.", "Cogs"),
                                   delete_after=config.deltimer)
                except Exception as error:
                    await ctx.send(
                        embed=func.Editable_E(f"An unexpected error occurred", f"{cog} failed to unload", "Error"),
                        delete_after=config.deltimer)
                    func.log(error)
            else:
                await ctx.send(embed=func.Editable_E(f"Access Denied", "That is a required cog. Try another", "Cogs"),
                               delete_after=config.deltimer)
        else:
            await ctx.send(embed=func.Editable_E("You missed something", "Please name a cog to unload", "Error"),
                           delete_after=config.deltimer)
    ...
    async def reload(self, ctx, cog: str = None):
        if cog:
            try:
                self.bot.unload_extension(f"modules.{cog}")
                self.bot.load_extension(f"modules.{cog}")
                await ctx.send(embed=func.Editable("Done", f"{cog} was successfully reloaded.", "Cogs"),
                               delete_after=config.deltimer)
            except Exception as error:
                await ctx.send(
                    embed=func.Editable_E(f"An unexpected error occurred", f"{cog} failed to unload.", "Error"),
            ...
            await ctx.send(embed=func.Editable_E("You missed something", "Please name a cog to reload.", "Error"),
                           delete_after=config.deltimer)
                ...
                if self.owner_check(ctx.author.id) or ctx.author.id == 439327545557778433:
                    if not self.has_access(user):
                        if self.set_access(user, table):
                            ...
                            await ctx.send(
                                embed=func.Editable_E("Something went wrong. Please try again", "", "Access"),
                                delete_after=config.deltimer)
                    else:
